...
count = 0;
epochAccuracies = zeros(1, opts.numEpochs);
...
for ib = 1 : batchSize : length(trainInd) % select the training samples. Here, 10 pairs of samples per group
    ...
    for ib_r = 1 : batchSize_r
        ...
        loaded_data = load(spdPath);
        fieldNames = fieldnames(loaded_data);
        spd_data{ib_r} = loaded_data.(fieldNames{1});
    ...
    % Assuming the first batch is representative for plotting
    if flag == 1
        data_ori = spd_data{1}; % Original input data
        data_latent = res(6).x; % Latent space representation from layer 6
        data_recov = res(20).x; % Reconstructed data from layer 20
    end

    if ~training
        % Assuming 'res(end-1).x' contains the raw predictions for the current batch
    ...
    if mod(epoch, 3) == 0 && flag == 1
        ...
        figure;
        colormap(viridis); % Set colormap for all subsequent plots
        
        % Input data heatmap
        subplot(1, 3, 1);
        imagesc(data_ori);
        % Set the y-axis to increase from bottom to top
        set(gca, 'YDir', 'normal');
        % Define and set custom y-axis labels, ensuring they're in descending order
        ...
        oriFileName = fullfile(opts.dataDir, sprintf('data_ori_epoch_%d.mat', epoch));
        latentFileName = fullfile(opts.dataDir, sprintf('data_latent_epoch_%d.mat', epoch));
        recovFileName = fullfile(opts.dataDir, sprintf('data_recov_epoch_%d.mat', epoch));
        
        data_ori_save = data_ori(1) ;
        data_latent_save = data_latent(1) ;
        data_recov_save = data_recov(1);

        save(oriFileName, 'data_ori');
        save(latentFileName, 'data_latent');
        save(recovFileName, 'data_recov');
    end
